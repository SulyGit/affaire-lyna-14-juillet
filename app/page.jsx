...
function MissionOne({ solved, complete, next }) {
  ...
  return (
    <MissionFrame mission={MISSIONS[0]} solved={solved} onNext={next} reveal={<><p className="reveal-time">Réservation · 13:15</p><h3>Fleur de Pavé</h3><p><strong>5 rue Paul-Lelong, 75002 Paris.</strong><br/>Le premier interrogatoire se fera à table. Arrive avec de l’appétit et un air innocent.</p></>}>
...
function MissionTwo({ solved, complete, next }) {
  ...
  return (
    <MissionFrame mission={MISSIONS[1]} solved={solved} onNext={next} reveal={<><p className="reveal-time">14:30 — 15:40</p><h3>Galerie Vivienne → Palais-Royal</h3><p>Et, caché près des arcades, un café chez <strong>Kitsuné</strong>. Le renard avait donc raison.</p></>}>
...
function MissionThree({ solved, complete, next }) {
  ...
  return (
    <MissionFrame mission={MISSIONS[2]} solved={solved} onNext={next} reveal={<><p className="reveal-time">15:50 accueil · 16:00 mission</p><h3>Lock Academy</h3><p>25 rue Coquillière. Dossier confié : <strong>Un Crime Presque Parfait</strong>. Briefing et débriefing compris.</p><div className="walk-note">15:40 — dix minutes de marche digestive</div></>}>
...
function MissionFour({ solved, complete, next }) {
  ...
  return (
    <MissionFrame mission={MISSIONS[3]} solved={solved} onNext={next} reveal={<><p className="reveal-time">17:15 départ · 17:45 cocktail</p><h3>Le bar de l’Hôtel Balzac</h3><p>6 rue Balzac. Une alcôve intime, un parfum littéraire, et le calme parfait avant le prochain chapitre.</p></>}>
...
function MissionFive({ solved, complete, next }) {
  ...
  return (
    <MissionFrame mission={MISSIONS[4]} solved={solved} onNext={next} reveal={<><p className="reveal-time">18:50 promenade · 19:30 dîner</p><h3>Avenue Mac-Mahon</h3><p>Le restaurant demeure sous scellés. Il sera révélé à l’arrivée — les meilleurs dossiers gardent un dernier secret.</p></>}>
...
function MissionSix({ solved, complete, next }) {
  const [sequence, setSequence] = useState([]);
  ...
  return (
    <MissionFrame mission={MISSIONS[5]} solved={solved} onNext={next} reveal={<><p className="reveal-time">20:35 départ · 21:00 coup d’envoi</p><h3>Le Royal Monceau</h3><p>Dernier acte : France–Espagne, dans un décor qui ne ressemble surtout pas à un pub.</p></>}>
  ...
  return (
    <section className="final-dossier">
      ...
      <div className="full-plan">
        <div className="plan-title"><span>ITINÉRAIRE DÉCLASSIFIÉ</span><b>14 JUILLET</b></div>
        {schedule.map(([time, place, detail], i) => <div className="plan-row" key={i}><time>{time}</time><div><b>{place}</b><small>{detail}</small></div></div>)}
      </div>
...
export default function Home() {
  ...
  useEffect(() => {
    try {
      ...
      if (Number.isInteger(saved.solved)) { setSolved(saved.solved); setCurrent(Math.min(saved.solved + 1, 7)); }
    } catch {}
  }, []);

  useEffect(() => {
    try { localStorage.setItem("lyna-case", JSON.stringify({ opened, solved })); } catch {}
  }, [opened, solved]);
  ...
  const next = id => { setCurrent(id); window.scrollTo({ top: 0, behavior: "smooth" }); };
  const reset = () => { localStorage.removeItem("lyna-case"); setOpened(false); setSolved(0); setCurrent(1); window.scrollTo({ top: 0, behavior: "smooth" }); };

  const mission = useMemo(() => {
    const props = { solved: solved >= current, complete: () => complete(current), next: current < 6 ? () => next(current + 1) : () => next(7) };
    ...
    return <FinalDossier onReset={reset}/>;
  }, [current, solved, soundOn]);
  ...
  return (
    <main className="case-shell">
      ...
      <div className="case-content">{mission}</div>
      <button className={`secret-fox ${fox >= 3 ? "discovered" : ""}`} onClick={() => setFox(v => Math.min(v + 1, 3))} aria-label="Petit renard caché">♢{fox >= 3 && <span>Indice secret : je t’aime.</span>}</button>
